...
Video clips support the same timeline operations as other media—drag to time, trim either edge, split at the playhead, and scrub to a specific frame. Frames are drawn from `trimIn + (projectTime - start)` during playback, scrubbing, and export.

Current limitations:

- **Audio track is not mixed into MP4 export.** Only the project `audio` element is written to the exported file.
